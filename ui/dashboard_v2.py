...
class DashboardV2:
    ...
    def print_startup_complete(self, stats: dict):
        ...
        rest_req = getattr(self.state, "rest_requests", 0)
        rest_429 = getattr(self.state, "rest_429s", 0)
        ws_reco = getattr(self.state, "ws_reconnect_count", 0)
        lines.append(f"[bold]REST:[/]     {rest_req} req ({rest_429}x 429) | WS↻ {ws_reco}")
