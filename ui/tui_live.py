...
class LiveHealth(Static):
    ...
    def render(self) -> str:
        ...
        rest_req = getattr(s, "rest_requests", 0)
        rest_429 = getattr(s, "rest_429s", 0)
        ws_reco = getattr(s, "ws_reconnect_count", 0)

        return (
            f"Runtime: {runtime}m | Rate: {rate_icon} WS↻{ws_reco}\n"
            f"Universe: {eligible} ({t1}ws/{t2}+{t3}rest)\n"
            f"Warm: {warm} | Cold: {cold}\n"
            f"Backfill: {backfills} pending\n"
            f"REST: {rest_req} req ({rest_429}x 429)\n"
            f"Budget: ${budget_used:.0f}/${budget_total:.0f}"
        )


class LiveProbes(Static):
    """Live probe monitoring - 50 probes, 1m checks, percentages."""
    ...
    def render(self) -> str:
        try:
            ...
            probes = probe_monitor.get_recent_probes(limit=12)
            stats = probe_monitor.get_stats()
            
            if not probes:
                return "[dim]No probes yet...[/]"
            
            lines = []
            lines.append(f"[dim]Probes: {stats['total_probes']} | W/L: {stats['winners']}/{stats['losers']} | Avg: {stats['avg_gain']:+.2f}%[/]")
            lines.append("[dim]SYM     AGE  %1M   VOL  SPRD[/]")
            
            for p in reversed(probes):  # Most recent first
                ...
                vol_color = "cyan" if p.vol_spike > 2 else "dim"
                spread_color = "green" if p.spread_bps < 20 else "yellow" if p.spread_bps < 40 else "red"
                
                sym = p.symbol.replace("-USD", "")[:6]
                lines.append(
                    f"[cyan]{sym:6}[/] {age:3}s [{pct_color}]{pct:+5.2f}%[/] [{vol_color}]{p.vol_spike:3.1f}x[/] [{spread_color}]{p.spread_bps:3.0f}[/]"
                )
            
            return "\n".join(lines)
        except Exception as e:
            return f"[red]Probe error: {e}[/]"


class LiveOrders(Static):
    """Live orders panel showing actual fills."""
    ...
    def render(self) -> str:
        orders = list(getattr(self.bot_state, "recent_orders", []))[-12:]
        
        if not orders:
            return "[dim]No orders yet[/]"
        ...
        for evt in orders:
            ...
            line = f"{ts_str} {icon} {sym:5}"
            
            if event_type == "open":
                line += f" OPEN ${price:.2f}"
            elif pnl is not None:
                pnl_color = "green" if pnl >= 0 else "red"
                line += f" [{pnl_color}]{pnl:+.2f}[/] @ ${price:.2f}"
            else:
                line += f" ${price:.2f}"
...
class LiveRecentSignals(Static):
    """Recent signals and gate decisions."""
    def __init__(self, state: BotState, **kwargs):
        super().__init__(**kwargs)
        self.bot_state = state

    def render(self) -> str:
        signals = list(getattr(self.bot_state, "recent_signals", []))[:15]
        if not signals:
            return "[dim]Waiting for signals...[/]"

        lines = ["[dim]SYM   STRAT   SCORE  SPREAD  STATUS[/]"]
        for ts, sym, strat, score, spread, taken, reason in signals:
            sym_short = sym.replace("-USD", "")[:6]
            status = "TAKEN" if taken else "BLOCK"
            status_color = "green" if taken else "yellow"
            spread_txt = f"{spread:.1f}bps" if spread else "—"
            reason_txt = (reason or "").replace("_", " ")[:24]
            lines.append(
                f"{sym_short:6} {strat[:7]:7} {score:5} {spread_txt:>7} [{status_color}]{status}[/] {reason_txt}"
            )
        return "\n".join(lines)
...
class LiveTradingDashboard(App):
    ...
    #scanner { border: solid cyan; padding: 0 1; min-height: 15; }
    #positions { border: solid green; padding: 0 1; min-height: 10; }
    #stats { border: solid magenta; padding: 0 1; }
    #verdicts { border: solid red; padding: 0 1; }
    
    #signal { border: solid yellow; padding: 0 1; }
    #recent_signals { border: solid yellow; padding: 0 1; }
    #health { border: solid blue; padding: 0 1; }
    #orders { border: solid white; padding: 0 1; min-height: 15; }
    #probes { border: solid cyan; padding: 0 1; min-height: 15; }
    
    #logs { column-span: 4; border: solid grey; padding: 0 1; min-height: 15; }
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            Label("🛰 Recent Signals", classes="title"),
            LiveRecentSignals(self.bot_state),
            id="recent_signals"
        )
        yield Container(
        ...
        yield Container(
            ...
            LiveOrders(self.bot_state),
            id="orders"
        )
        yield Container(
            Label("🔍 Probes", classes="title"),
            LiveProbes(self.bot_state),
            id="probes"
        )
